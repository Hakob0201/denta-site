...
{{-- @if($headlines->has($cats[2] ?? 0))
    <section class="px-16">
        <div class="container">
            ...
            <div class="grid grid-cols-6 lg:gap-30">
                <a class="other-big relative col-span-6 lg:col-span-3" href="/{{ $locale }}/article/{{ $headlines->get($cats[2])->first()->article_id }}">
                    @include('includes.articles.image', ['item' => $headlines->get($cats[2])->first(), 'type' => 'w-', 'class' => 'w-full h-full'])
                    <div class="absolute inset-0 bg-black bg-opacity-30">
                        <div class="content text-white absolute bottom-0 p-26">
                            <h2 class="mb-12 lg:mb-22 text-18 lg:text-24">{{ $headlines->get($cats[2])->first()->title }}</h2>
                            <time class="relative-date long text-15" datetime="{{ dateF($headlines->get($cats[2])->first()->datetime_at) }}">{{ $headlines->get($cats[2])->first()->datetime_at }}</time>
                        </div>
                    </div>
                </a>
                <div class="grid grid-cols-6 col-span-6 bg-white shadow lg:bg-none lg:shadow-none lg:gap-30 lg:col-span-3">
                    @foreach($headlines->get($cats[2])->slice(1, 6) as $item)
                        <a class="col-span-6 lg:col-span-3 text-15 border-b border-primary-20 border-opacity-50 pt-22 px-16 lg:pt-0 lg:px-0" href="/{{ $locale }}/article/{{ $item->article_id }}">
                            <h3 class="text-primary-100 font-medium">{{ $item->title }}</h3>
                            <time class="relative-date long block text-orange py-16" datetime="{{ dateF($item->datetime_at) }}">{{ $item->datetime_at }}</time>
                        </a>
                    @endforeach
                </div>
            </div>
